...
class VideoObjectAdminTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.stack = get_stack()
    ...
    def test_stored_dangerous_urls_render_as_inert_text_never_as_links(self):
        js_id = seed_with(self.stack, ENTITY, {"contentUrl": "javascript:alert(1)"}, self.jar)
        ...
        data_id = seed_with(self.stack, ENTITY, {"contentUrl": "data:text/html,x"}, self.jar)
    ...
    def test_stored_http_url_renders_as_a_clickable_link(self):
        item_id = seed_with(self.stack, ENTITY, {"contentUrl": "https://example.com/profile"}, self.jar)
